...
n=$(sed '/^## Staged/,$d' tasks/MASTER_BACKLOG.md 2>/dev/null | grep -cE '^\| ([0-9]+\.[0-9]+[ab]?|X\.[0-9]+) \|' || true); [ "${n:-0}" -eq 154 ] && ok "MASTER_BACKLOG = 154 doc-38 rows (v4.6, D-187)" || miss "MASTER_BACKLOG rows = $n (expected 154 — run: python3 scripts/gen-backlog.py)"
