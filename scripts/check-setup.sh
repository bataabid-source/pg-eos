...
n=$(sed '/^## Staged/,$d' tasks/MASTER_BACKLOG.md 2>/dev/null | grep -cE '^\| ([0-9]+\.[0-9]+[ab]?|X\.[0-9]+) \|' || true); [ "${n:-0}" -eq 137 ] && ok "MASTER_BACKLOG = 137 doc-38 rows (v4.5, D-185)" || miss "MASTER_BACKLOG rows = $n (expected 137 — run: python3 scripts/gen-backlog.py)"
