...
| 2 | 2.9b DONE @ `716bf8e` — module locks `wms/schedule-inbound`/`wms/receive-inbound` released; next lane-2 task pending Master assignment | — | `../pg-eos-lane-2` | idle, awaiting next lock |
| 3 | 3.17 part 1 done-in-part @ `<this commit>` — DTL gate evaluation mechanism, PASS(27 findings/4 rounds); DEL_MGR further parts/auditor screen/nightly job not yet scheduled; lane 3 next: awaiting the Master's `tms` claim for 3.1 (D-184) | `imile` | `../pg-eos-lane-3` | own worktree (hook-enforced); real 3.17 acceptance (§5-3 auditor screen, 01:40 nightly job, ADR-27, OCR vendor) still out of scope |
...
3. Lane 3: 3.17 part 1 done-in-part @ `<this commit>` — lane 3 next: awaiting the Master's `tms` claim for 3.1 (D-184)
