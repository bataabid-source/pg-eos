...
|---|---|---|---|---|
| 1 | 2.11 part 2 DONE-in-part @ `<this commit>` → 2.11 part 3 (describe names + 2.4 picker) → 2.12 | `wms/process-outbound` + `wms/receive-inbound` + `wms/integration` (tests only) | `../pg-eos-lane-1` | **runnable now** — part 2 closed-in-part, same lock continues to part 3 |
...
| task | commit |
|---|---|
| 2.11 part 2 — outbound allocation: Allocate/GeneratePickList/cancel-release, single-lot FEFO/FIFO, no migration (lane 1), DONE-in-part — round 3 FAIL(1 low) carried to part 3; round 3 taken by the Master on opus (D-186 two-round cap reached) | `<this commit>` |
