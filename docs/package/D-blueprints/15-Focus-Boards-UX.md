...
> **تعديل GM — D-145 (24/09/2026):** يُضاف **تنبيه صوتي على لوحات الويب** لكل مسؤولي التشغيل والميدانيين (`WH_MGR · WH_SUP · DEL_MGR · DEL_SUP · FLEET_MGR · CC_MGR`). القاعدة تبقى «لا نافذة منبثقة»: الصوت يرافق **وصول بطاقة جديدة** من تنبيه `schedule = realtime` موجَّه إلى الدور، مرة واحدة لكل بطاقة، ويحترم ساعات الهدوء وحقل الكتم في `alert_rules`؛ لا صوت لأدوار الإدارة العليا والمالية. **D-167 (Q17 ب): الصوت يشمل كل تنبيه موجَّه لأدوار التشغيل، الفوري والدوري معاً؛ `alert_rules.sound` يُبذر `true` لهذه الأدوار ويعدّله أدمن النظام لكل قاعدة.** الحامل المقترح: عمود `alert_rules.sound boolean default false` يضبطه أدمن النظام من شاشة 5.13 (SCR-HR-SHIFT-01 §2.10). لا تفضيل لكل مستخدم (D-138).
