...
## 10. المهمة القادمة حسب /resume
1. تبعية 2.6 (أي 1.5) صار لها هاش في git (`f790da7`)، والشجرة نظيفة، ولا أقفال — فشرط /resume «every dependency DONE with a hash» متحقق.
2. الصفوف READY بترتيب الجدول: 1.2 أولاً، لكن معيار قبولها «rejected from UI, API and import» غير قابل للتشغيل (لا apps/ ولا API ولا شريحة ذهبية) = عائق حقيقي حسب /resume. 3.1 و3.3 تحتاجان مسار تعيين (جداول TMS/HR)، و3.14 و5.13 و6.4 تحتاج خدمات وواجهات غير موجودة.
