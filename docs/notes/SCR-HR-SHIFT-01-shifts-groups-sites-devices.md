...
**2.5 Device registry link** (extends SCR-HR-ATT-01 §2.2, one registry in `identity`): add `asset_id → admin.assets (nullable: personal phone)`, `ownership ∈ {company · personal}`, `enrolled_at` (biometric set up at handover, D-143 line 1), `handover_custody_id → admin.asset_custody (nullable)`. Rule: a **company** phone must have an open custody row before it can be registered; a **personal** phone is registered without custody. **D-145:** add `push_token text` (provider token for push notifications, rotated on app reinstall) and `push_provider ∈ {fcm · apns}` to the same registry row — the carrier the Premium Decisions and driver-app notifications need and that 01/13/13B lack; classified `secret` (never reaches the browser).
...
**2.10 `platform.alert_rules.sound boolean not null default false`** (D-145) — when true, a realtime alert that lands as a new card on a web board of an operational role plays one sound; honours `quiet_hours` and `muted_until`. Set by SYSADMIN from the alert-admin screen (5.13). No per-user preference (D-138).
