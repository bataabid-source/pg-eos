...
| `gen-backlog.py` | Generates `tasks/MASTER_BACKLOG.md` from `docs/package/38-WBS.md`: 137 rows (v4.5, 2.9b added D-185; 5.5a + 5.18 added D-173; 5.3b D-167; D-124), IDs · type markers · deps · lane · owner · acceptance copied verbatim + a `Status` column (TODO · READY · ACTIVE · WAITING_GM · BLOCKED · DONE @ hash · SUPERSEDED · DEFERRED-POST-PILOT, D-127). Rerunnable — existing statuses are preserved; `--check` verifies the count without writing. |
