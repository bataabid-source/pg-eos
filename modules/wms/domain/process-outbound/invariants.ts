 ...
 *  including blocked ones (brief Master decision 3), must cover the ordered quantity. */
export function assertSufficientStock(check: StockSufficiencyCheck): void {
  if (Number(check.availableSum) < Number(check.ordered)) {
...
export function assertNonBlockedLocationsSufficient(
  ...
  const nonBlockedAvailable = locations
    ...
    .reduce((sum, location) => sum + Number(location.qtyAvailable), 0);
  if (nonBlockedAvailable < Number(ordered)) {
