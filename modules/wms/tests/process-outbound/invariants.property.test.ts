...
describe('assertNonBlockedLocationsSufficient — property with FRACTIONAL 3-decimal quantities (finding 7, exact Quantity arithmetic)', () => {
  ...
  it('always throws LocationBlockedError when ordered exceeds the EXACT non-blocked sum by as little as 0.001, reporting the exact available sum', () => {
    fc.assert(
      fc.property(
        ...
        (locations, deficitThousandths) => {
          ...
          expect((error as LocationBlockedError).params).toMatchObject({ available: thousandthsToQty(exactSum), ordered });
        },
      ),
    );
  });
