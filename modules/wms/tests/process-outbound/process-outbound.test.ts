...
const ROLE_ACTOR_UUID = '00000000-0000-4000-8000-0000000211a1';
const NO_ROLE_ACTOR_UUID = '00000000-0000-4000-8000-0000000211a2';
const OUTSIDER_ACTOR_UUID = '00000000-0000-4000-8000-0000000211a3'; // RLS: no user_entities row for entityId.
...
async function insertPriceList(opts: { readonly withOf01Price?: boolean } = { withOf01Price: true }): Promise<string> {
  ...
  const listResult: QueryResult<{ id: string }> = await pool.query(
    ...
    [entityId, code, 'قائمة تسعير اختبار صرف صادر'],
  );
...
async function insertContract(
  ...
  const result: QueryResult<{ id: string }> = await pool.query(
    `insert into sales.contracts (entity_id, doc_no, account_id, title, status, start_date, end_date, price_list_id)
     values ($1, $2, $3, $4, $5, current_date - interval '1 year', $6::date, $7) returning id`,
    [
      entityId,
      docNo,
      clientId,
...
// 019-Warehouse-WH1-Setup.sql chk_locations_code_format: code matches `^[PGMT][1-9]-[0-9]{2}-[1-9]$`
// for pallet/shelf locations — same reserved 'T9-xx-8/9' block the receive-inbound WBS 2.10 fixture
// uses, collision-free with WH1's own real layout.
...
afterAll(async () => {
  ...
  if (fixtureOrderIds.length > 0) {
    ...
    await pool.query(`delete from wms.outbound_orders where id = any($1::uuid[])`, [fixtureOrderIds]);
  }
  for (const clientId of fixtureClientIds) {
    // stock_movements before stock_balance — same ordering discipline as count-inventory.test.ts's
    // own afterAll (a ledgerless balance row surviving cleanup would itself fail G1 on the next run).
  ...
  if (fixtureLocationIds.length > 0) await pool.query(`delete from wms.locations where id = any($1::uuid[])`, [fixtureLocationIds]);
  if (fixtureZoneIds.length > 0) await pool.query(`delete from wms.zones where id = any($1::uuid[])`, [fixtureZoneIds]);
  if (fixtureContractIds.length > 0) await pool.query(`delete from sales.contracts where id = any($1::uuid[])`, [fixtureContractIds]);
  if (fixturePriceListIds.length > 0) {
    await pool.query(`delete from catalog.price_list_lines where price_list_id = any($1::uuid[])`, [fixturePriceListIds]);
    await pool.query(`delete from catalog.price_lists where id = any($1::uuid[])`, [fixturePriceListIds]);
  }
  if (fixtureClientIds.length > 0) await pool.query(`delete from sales.accounts where id = any($1::uuid[])`, [fixtureClientIds]);
  ...
  await pool.end();
  await appPool.end();
}, 30000); // the ledger-seeding fixtures (G1 fix) add many more fixture rows to unwind per run
// than the default 10s hook timeout comfortably covers — same headroom count-inventory.test.ts's
// own afterAll needs for the same reason.
...
describe('Scenario: Create a draft order', () => {
  ...
  it('a failing command (unqualified client) writes NEITHER an outbox row NOR an audit row — the transaction rolls back', async () => {
    const clientId = await insertClient({ deletedAt: new Date() });
    const correlationId = nextCorrelationId();
    await expect(
...
describe('Scenario: Condition 7 fails — every candidate location is blocked', () => {
  it('rejects with LocationBlockedError (i18nKey wms.outbound.check.locationBlocked)', async () => {
    const clientId = await insertClient();
    const priceListId = await insertPriceList();
    const contractId = await insertContract(clientId, { priceListId });
    const skuId = await insertSku(clientId);
    ...
    expect(error).toBeInstanceOf(LocationBlockedError);
    expect((error as LocationBlockedError).i18nKey).toBe('wms.outbound.check.locationBlocked');
    ...
    // param `blockReason`.
    expect((error as LocationBlockedError).params).toMatchObject({
