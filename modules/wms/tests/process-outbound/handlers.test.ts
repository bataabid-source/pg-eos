...
const FIXTURE_ACTOR_UUID = '00000000-0000-4000-8000-0000000211c1';
...
beforeAll(async () => {
  ...
  const priceListResult: QueryResult<{ id: string }> = await pool.query(
    `insert into catalog.price_lists (entity_id, code, name_ar, valid_from, status)
     values ($1, $2, $3, current_date - interval '1 year', 'active') returning id`,
    [entityId, `_procout_handlers_pl_${randomUUID()}`, 'قائمة تسعير اختبار معالجات الصرف'],
  );
  ...
  const contractResult: QueryResult<{ id: string }> = await pool.query(
    `insert into sales.contracts (entity_id, doc_no, account_id, title, status, start_date, price_list_id)
     values ($1, $2, $3, $4, 'active', current_date - interval '1 year', $5) returning id`,
    [entityId, docNoCntResult.rows[0]?.doc_no, fixtureClientId, 'عقد اختبار معالجات الصرف', priceListId],
  );
...
afterAll(async () => {
  if (checksPendingOrderId) {
    await pool.query(`delete from wms.order_lines where order_id = $1`, [checksPendingOrderId]);
    await pool.query(`delete from wms.outbound_orders where id = $1`, [checksPendingOrderId]);
  }
  if (extraOrderIds.length > 0) {
    await pool.query(`delete from wms.order_lines where order_id = any($1::uuid[])`, [extraOrderIds]);
    await pool.query(`delete from wms.outbound_orders where id = any($1::uuid[])`, [extraOrderIds]);
  }
  // the "no price for OF-01" test posts a real ledger receipt (G1 fix) for fixtureSkuId —
  // stock_movements before stock_balance, both before the SKU itself (stock_balance_sku_id_fkey).
  if (fixtureClientId) {
  ...
  if (priceListId) {
    await pool.query(`delete from catalog.price_list_lines where price_list_id = $1`, [priceListId]);
...
describe('a missing Idempotency-Key is rejected with a 400 Problem', () => {
  it('handleApproveOutbound: no Idempotency-Key header -> 400, title reflects the rejection', async () => {
    const result = await handleApproveOutbound(
      requestWithoutKey({ orderId: checksPendingOrderId, expectedVersion: checksPendingOrderVersion, correlationId: randomUUID() }),
      deps,
    );
...
describe('StaleVersionError maps to 409, title = error.name', () => {
  it('handleApproveOutbound: a stale expectedVersion -> 409, title "StaleVersionError"', async () => {
    const result = await handleApproveOutbound(
      requestWithKey({ orderId: checksPendingOrderId, expectedVersion: checksPendingOrderVersion + 999, correlationId: randomUUID() }),
      deps,
    );
...
describe('IllegalTransitionError maps to 422, title = error.name', () => {
  it('handleApproveOutbound on a still-draft order -> 422, title "IllegalTransitionError"', async () => {
    ...
    const result = await handleApproveOutbound(
      requestWithKey({ orderId: draft.id, expectedVersion: draft.version, correlationId: randomUUID() }),
      deps,
    );
...
describe('a condition-check failure maps to 422 carrying the i18n key + params in the Problem body', () => {
  it('handleRunOutboundChecks: no price for OF-01 -> 422, title "NoServicePriceError", i18nKey in the body', async () => {
    ...
    const unpricedPriceListResult: QueryResult<{ id: string }> = await pool.query(
      `insert into catalog.price_lists (entity_id, code, name_ar, valid_from, status)
       values ($1, $2, $3, current_date - interval '1 year', 'active') returning id`,
      [entityId, `_procout_handlers_unpriced_${randomUUID()}`, 'قائمة بدون سعر'],
    );
    ...
    const unpricedContractResult: QueryResult<{ id: string }> = await pool.query(
      `insert into sales.contracts (entity_id, doc_no, account_id, title, status, start_date, price_list_id)
       values ($1, $2, $3, $4, 'active', current_date - interval '1 year', $5) returning id`,
      [entityId, docNoPlResult.rows[0]?.doc_no, fixtureClientId, 'عقد بدون سعر', unpricedPriceListId],
    );
    ...
    const draft = draftResult.rows[0] as { id: string; version: number };
    // NOT pushed to the shared extraOrderIds — this order references unpricedContractId, which
    // this test deletes itself (below) before afterAll's own contract-independent cleanup runs;
    // the order (and its lines) must be gone BEFORE the contract it FKs to, so it is deleted here,
    // inline, in that order.
    await pool.query(
    ...
    const locationResult: QueryResult<{ id: string }> = await pool.query(
      `select id from wms.locations where warehouse_id = $1 and is_blocked = false limit 1`,
      [warehouseId],
    );
    // try/finally: cleanup runs even if an assertion below throws — an earlier round of this test
    // left the order/lines behind on an assertion failure, tripping order_lines_sku_id_fkey on the
    // NEXT run's afterAll (fixtureSkuId delete).
    try {
      await postMovement(
        ctx,
        {
          ...
          entry: {
            ...
            skuId: fixtureSkuId,
            fromLocationId: null,
            toLocationId: (locationResult.rows[0] as { id: string }).id,
      ...
      // Order (and its lines) before the contract it FKs to, before the price list — same ordering
      // as the module's other cleanup blocks (order_lines -> outbound_orders -> contracts -> price_lists).
      await pool.query(`delete from wms.order_lines where order_id = $1`, [draft.id]);
      await pool.query(`delete from wms.outbound_orders where id = $1`, [draft.id]);
      await pool.query(`delete from sales.contracts where id = $1`, [unpricedContractId]);
      await pool.query(`delete from catalog.price_lists where id = $1`, [unpricedPriceListId]);
    }
  });
});
