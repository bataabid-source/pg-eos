...
//
// Lock order: (1) order-row lock + expectedVersion check, (2) role gate, (3) CloseBlockedError
// business-rule gate (any line still open), (4) machine legality check (via canTransition),
// (5) the unconditional version bump with closed_at/closed_by, (6) audit row (last).
//
// Legality comes from the machine alone: CLOSE is legal from 'putaway' and from 'received' (the
// zero-qty case — an order whose lines all received qty 0 never needs a put-away). Whether the
// order MAY close is then the separate business rule below: no line may still be open.
    ...
    if (!canTransition(order.status, INBOUND_ORDER_EVENTS.CLOSE)) {
      throw new IllegalTransitionError(
        `CloseInbound is illegal from status "${order.status}" (the machine allows CLOSE only from ` +
          `"putaway", or from "received" when no line needed a put-away).`,
      );
    }
