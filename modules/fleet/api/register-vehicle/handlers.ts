...
/** Builds the IdempotencyInput the write handler passes to registerVehicle as `idem`. `entityId`
 *  is null — the command itself resolves the entity; the key row's entity scope is optional. Called
...
function buildIdem(request: ApiRequest<unknown>, endpoint: string, parsedBody: unknown): IdempotencyInput {
  ...
  if (!key) {
    ...
    // it is missing. Thrown, not silently defaulted, if that ever drifts.
