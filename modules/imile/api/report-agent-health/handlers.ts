...
/** Builds the IdempotencyInput the write handler passes to reportAgentHealth as `idem`. `entityId`
 *  is null — imile.agent_health carries no entity_id column (brief §2). Called only after
 ...
 *  below; reportAgentHealth's own input also carries `correlationId`, but a 500 can happen before
