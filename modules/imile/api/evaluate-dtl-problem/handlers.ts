...
/** Builds the IdempotencyInput the write handler passes to evaluateDtlProblem as `idem`. `entityId`
 *  is null — imile.dtl_problems has no entity_id column (brief, Design). Called only after
 ...
 *  below; evaluateDtlProblem's own input also carries `correlationId`, but a 500 can happen before
