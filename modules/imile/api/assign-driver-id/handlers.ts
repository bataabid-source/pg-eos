// modules/imile/api/assign-driver-id/handlers.ts — WBS 3.12.
//
...
// packages/contracts/imile/assign-driver-id.ts, an Idempotency-Key header required on the write
// (CLAUDE.md · ARCHITECTURE), every typed domain error mapped to the RFC 9457 Problem envelope. A
// future NestJS controller wraps this one-to-one; no business logic lives here.
//
// Error mapping:
...
//   - DriverIdNotAvailableError / EmployeeAlreadyAssignedError both map to 409 (brief: "both map to
//     409 at the API layer — a real conflict, not a validation failure");
...
// it to assignDriverId as `idem` — the command itself runs its DB-writing transaction inside
...
// `entityId` is always null — neither imile.driver_id_assignments nor imile.driver_ids has an
// entity_id column (brief, Event: "neither table has an entity_id column").
...
/** Builds the IdempotencyInput the write handler passes to assignDriverId as `idem`. `entityId` is
 *  null — neither table has an entity_id column (brief, Event). Called only after
 ...
 *  below; assignDriverId's own input also carries `correlationId`, but a 500 can happen before the
...
async function handle<TBody>(
  ...
  try {
    ...
    if (failure.status === HTTP_STATUS_INTERNAL_SERVER_ERROR) {
      deps.logger.error({ correlationId, err: error }, 'imile.assign-driver-id: unhandled error');
    }
    return failure;
  }
}

export async function handleAssignDriverId(
  request: ApiRequest<unknown>,
  deps: AssignDriverIdDeps,
): Promise<ApiResult<Awaited<ReturnType<typeof assignDriverId>>>> {
  const missingKey = requireIdempotencyKey(request.headers);
  if (missingKey) return missingKey;
  return handle(
    () => {
      const input = AssignDriverIdInputSchema.parse(request.body);
      const idem = buildIdem(request, IDEMPOTENCY_ENDPOINT_ASSIGN, input);
      return assignDriverId(request.ctx, { ...input, idem }, deps);
    },
    deps,
