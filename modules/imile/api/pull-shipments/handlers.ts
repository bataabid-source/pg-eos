...
/** Builds the IdempotencyInput the write handler passes to pullShipments as `idem`. `entityId` is
 *  null — a pull cycle touches many shipments, not one entity. Called only after
 ...
 *  below; pullShipments's own input also carries `correlationId`, but a 500 can happen before the
