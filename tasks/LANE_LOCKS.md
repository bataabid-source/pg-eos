...
|---|---|---|---|---|
| wms/process-outbound | 1 | 2.11 | 2026-09-25 | ../pg-eos-lane-1 (2.11 part 2 DONE-in-part @ `<this commit>` — Allocate/GeneratePickList/cancel-release, single-lot FEFO/FIFO; review round 3 FAIL(1 low) carried to part 3: three `describe` names ≠ Gherkin scenario names; migration 0022 WITHDRAWN — column already in 13B; 2.10 DONE @ `95a33b8`) |
| wms/receive-inbound | 1 | 2.11 | 2026-09-25 | ../pg-eos-lane-1 (2.11 part 2 DONE-in-part @ `<this commit>` — test-files-only grant, item 0 complete: `pickFreshWh1Location`/`nextFixtureLocationCode()` now create their own M9 locations instead of selecting shared/orphan rows; row kept open into part 3 per the Master's instruction; no domain/application/api file) |
...
`fleet` (lane 3, task 3.1) released 2026-09-26 — 3.1 both use cases DONE-in-part: part 1 `RegisterVehicle` @ `e3137d5` (PASS, 0 open round 2), part 2 `AssertVehicleAssignable` (the real INV-C4-1 gate) @ `<this commit>` (PASS(1 finding, 2 rounds)). Judgment call recorded in CHANGELOG/MASTER_BACKLOG: the gate mechanism is complete/tested/proven but not yet called by any real assignment write path — that wiring is a future delivery/TMS slice's job (G-01, `tasks/backlog/MIGRATION-REQUEST-3.md`), not this row's own scope. Lane 3 idle, next task pending the Master's assignment (3.4, once 2.12 DONE).
`wms/integration` (lane 1, task 2.11 part 2/3) released 2026-09-25 — its only item, `modules/wms/tests/integration/location-limits.test.ts` `pickWh1Location` (WBS 2.4), is fixed to the collision-safe dedicated-`M9-`-location pattern in 2.11 part 3 @ `<this commit>` (tests-only grant, no domain/application/api file touched); `wms/process-outbound` + `wms/receive-inbound` stay held for 2.11 part 4 (2 deferred naming items; condition 10 stays BLOCKED, `docs/notes/SCR-WMS-OUT-02-order-quantity-limit.md`).
...
`imile` (lane 3, tasks 3.14 / 3.17) released 2026-09-25 — 3.14 part 2 DONE-in-part @ `e667821`, 3.17 part 1 DONE-in-part @ `ca7f6f1`; lane 3 re-claimed above for `tms` (3.1, D-184). Former row note: ../pg-eos-lane-3 (part 1, DTL gate evaluation mechanism, DONE-in-part @ `<this commit>`; 3.14 part 2 DONE-in-part on main; lane 3 next awaits the Master's `tms` claim for 3.1 per D-184)
