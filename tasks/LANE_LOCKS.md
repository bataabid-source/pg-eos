...
| fleet | 3 | 3.1 | 2026-09-25 | ../pg-eos-lane-3 (`tms.vehicles`, documents, hard gate on expired docs — doc-38 Lane 1 row reassigned to lane 3 by the GM, D-184; whole-module lock: `modules/fleet` (fleet.brief.md — vehicles, vehicle_documents, maintenance, fuel, accidents live in schema `tms` but belong to the `fleet` module; `tms` is the delivery module, unclaimed) does not exist yet and is created by `scripts/new-slice.sh fleet register-vehicle`; corrected 2026-09-25 from a Master mis-naming (`tms`) caught by lane 3; brief ≤ 8 files / 1,000 lines, two review rounds (D-186); `fleet.*` events go to the Master for the catalog) |
