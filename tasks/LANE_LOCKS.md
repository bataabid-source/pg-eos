...
| wms/receive-inbound | 1 | 2.11 | 2026-09-25 | ../pg-eos-lane-1 (test-files-only grant for 2.11 part 2's first item: `modules/wms/tests/receive-inbound/receive-inbound.test.ts` `pickFreshWh1Location` creates its own M9 location instead of `order by code desc limit 1` on shared rows — the 2.10 fixture defect pg-reviewer traced in 2.9b round 5; no domain/application/api file; released with 2.11 part 2) |
| wms/integration | 1 | 2.11 | 2026-09-25 | ../pg-eos-lane-1 (tests-only grant: `modules/wms/tests/integration/location-limits.test.ts` `pickWh1Location` (WBS 2.4) selects orphan shared rows by `order by code desc limit 1` — same class as the two receive-inbound pickers; fixed by lane 1 in 2.11 part 2 or its first follow-up; no domain/application/api file; released with 2.11 part 2) |
