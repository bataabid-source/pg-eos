...
| tms | 3 | 3.1 | 2026-09-25 | ../pg-eos-lane-3 (`tms.vehicles`, documents, hard gate on expired docs — doc-38 Lane 1 row reassigned to lane 3 by the GM, D-184; whole-module lock: `modules/tms` does not exist yet and is created by `scripts/new-slice.sh tms register-vehicle`; brief ≤ 8 files / 1,000 lines, two review rounds (D-186); `tms.*` events go to the Master for the catalog) |
