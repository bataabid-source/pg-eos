...
| 2.11 | Outbound order: ten-condition check, FEFO allocation, pick sequence (A3) | 🤖 | 2.9, 1.8 | **1** | WH_MGR | Each of ten conditions has a failing test with the correct message | IN PROGRESS — lane 1 (D-180), migration 0022 issued |
...
| 3.14 | iMile station agent (dedicated account, remote UI operator, health reporting) | 🤖 | 0.12 | **3** | DEL_MGR | Pull every 10 min; stop > 15 min alerts | IN PROGRESS — part 1 @ `<this commit>` (health-reporting mechanism: `ReportAgentHealth` command, domain invariants, append-only insert into `imile.agent_health`, Idempotency-Key, RLS-protected read, wired to the existing N-01 alert rule; no new migration); part 2 @ `<this commit>` (`PullShipments` command — portal pull mechanism, optimistic lock + StaleVersionError, agent_health heartbeat, audit_log rows, Idempotency-Key, migration 0024 `imile.shipments.version`, `NotConfiguredImilePortalAdapter` stub gated on D-149; PASS(27 findings/7 rounds)) — full acceptance not met: real Playwright/live-portal adapter, pg-boss 10-min schedule, and the >15-min alert remain out of scope pending the D-149 external check |
