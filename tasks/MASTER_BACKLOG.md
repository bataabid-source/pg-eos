...
| 0.18 | **RLS enabled on every operational table in 01/13/13B/019** (the patterns in 01 §11 and 13B §12 generalised) + **client isolation test** (ID tampering) | ✅ | 0.17 | **M** | SYSADMIN | **G7 returns 0** · User A returns zero rows from B's data on direct ID substitution, with no error | BLOCKED — partial delivery, NOT DONE @ `<pending>`. **Delivered:** `tests/isolation` workspace (`@pg-eos/isolation-tests`), G14 runner (`pnpm test:isolation`) wired into `scripts/guards-run.sh` + turbo routing, eslint scoped to tests. Suite 21/24 pass; 3 pinned FAILS (see below). No migration, no schema change. **Blockers (G-01 schema-change requests, both reproduced, both open pending GM):** (1) SCR-RLS-01 — `entity_scope` (FOR ALL) OR `client_portal_scope` (FOR SELECT) defeats isolation on 7 tables (billing.invoices, tms.delivery_tasks, wms.outbound_orders, wms.occupancy_snapshots, wms.space_allocations, wms.space_reservations, wms.work_orders); portal user with `user_entities` row reads another client's data — reproduced 1 row where 0 demanded. (2) SCR-RLS-02 — `platform.audit_log` partitioned parent (`relkind='p'`) has no RLS, bypasses G7 filter (`relkind='r'`); NOBYPASSRLS role read all 8 rows. **Carried forward:** SCR-RLS-01, SCR-RLS-02, apply.sh line 207 needs master edit, 0.17 identity.users fixture leftover, runtime still uses superuser (production 0.5/0.6 work), turbo `test` task caches DB-touching integration tests and is unhashed on `PG*` (gates ②/③ can replay a stale green — fix with `"env"` or `cache: false`, same single-lane task as apply.sh:207). |
