...
**136 doc-38 tasks (v4.4: 0.6 → 0.6a / 0.6b, D-124; 5.3b added, D-167; 5.5a + 5.18 added, D-173) + X.1–X.6 CONTINUOUS + 0.19 DEFERRED + DEFERRED-POST-PILOT rows (D-127)** — eight phases (0–7) + cross-cutting X-tasks; X.1–X.6 and 0.19 are counted inside the 133. No 18-phase roadmap, no separate database phase (BOOTSTRAP-v4 §8).
...
| `SUPERSEDED — <id>` | row kept for the 136 count; replaced by the named ADR / GM decision, never picked (D-125) |
...
| 0.19 | Admin app shell: navigation, Decision Inbox, empty-state component, design system (lane 1 after 1.2 — D-172) | 🤖 | 0.17 | **1** | SYSADMIN | Inbox renders; empty state shows "what's missing + owner" | DONE @ `<this commit>` — lane 1; UI shell only, mock data client (platform lock held by lane 2 at build time); decide-wiring, real endpoint and packages/i18n promotion are follow-up slices. |
...
**Rows: 136** (expected 136 — `--check` fails otherwise; the four Staged rows above are never part of this count).
