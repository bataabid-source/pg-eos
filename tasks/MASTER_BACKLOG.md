...
| ID | Task | Type | Depends on | Lane | Owner | Acceptance | Status |
|---|---|---|---|---|---|---|---|
| 3.1 | `tms.vehicles`, documents, hard gate on expired docs | 🤖 | 0.9 | **1** | FLEET_MGR | Expired-doc vehicle cannot be assigned | IN PROGRESS — part 1 @ `<this commit>` (`RegisterVehicle` + documents — vehicle + document registration, insert-only, `canBeAssigned` a non-authoritative snapshot; PASS(7 findings fixed round 1, 0 open round 2)); part 2: `AssertVehicleAssignable` (the real INV-C4-1 gate) + 6 round-2 findings (brief-compliance/citation-hygiene class only, no architectural rework needed) — split from part 1 per the Master's fairness rule since register-vehicle itself had zero open findings. **Acceptance "Expired-doc vehicle cannot be assigned" is NOT met yet** — part 1 only registers vehicles/documents, it does not gate anything. Lane 3 (D-184: doc-38 lane 1 row reassigned by the GM); `modules/fleet` created by new-slice.sh; budget 8 / 1,000, two rounds (D-186) |
