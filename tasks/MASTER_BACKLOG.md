...
| 3.14 | iMile station agent (dedicated account, remote UI operator, health reporting) | 🤖 | 0.12 | **3** | DEL_MGR | Pull every 10 min; stop > 15 min alerts | IN PROGRESS — part 1 @ `<this commit>` (health-reporting mechanism: `ReportAgentHealth` command, domain invariants, append-only insert into `imile.agent_health`, Idempotency-Key, RLS-protected read, wired to the existing N-01 alert rule; no new migration); part 2 = `services/agent` (Node + Playwright, dedicated iMile account, 10-min pull loop, single-session enforcement against the live portal) — full acceptance not met until part 2 |
