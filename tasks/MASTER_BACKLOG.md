...
| 5.5a | Shifts, shift groups, work sites — `hr.shifts`, `hr.shift_assignments`, `hr.shift_groups`, `platform.sites` (SCR-HR-SHIFT-01 §2.1–§2.4; pulled into the pilot by D-144, row issued by the Master D-173) | 🤖 | 3.3 | **2** | HR_MGR | One active shift assignment per employee per date (exclusion constraint on the date range); `platform.sites` is the single sites table (`kind ∈ {warehouse · office · client_pickup · housing · other}`, `radius_m` defaulting from `platform.thresholds` `att.geofence_radius_m`) referenced by `hr.employees.default_site_id`; a shift group carries its own attendance site and lead driver; every state change → outbox + audit in one transaction; G1, G6, G9, G11 green | IN PROGRESS — **part 1 DONE @ `<this commit>`** (`platform.sites`, migration 0015, review PASS(12 findings fixed)); **part 2 pending** (`hr.shifts` / `hr.shift_assignments` / `hr.shift_groups`, own migration number, own brief) — lane 2 next; `platform` lock released to the Master, `hr` lock stays with lane 2 |
