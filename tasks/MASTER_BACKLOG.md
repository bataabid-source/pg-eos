...
| 2.9 | **GOLDEN SLICE — Receive inbound order** (PDA + state machine + ledger + event + GRN + billable events) | 🤖 | 2.4, 2.6, 2.8, 0.15 | **M** | WH_MGR + GM review | Full human review; becomes the template | IN PROGRESS — part 2 @ `<this commit>`; GM sheet-3 answers applied (idempotency store, cancel/close rules, variance photo, shared logger, D-150…D-158); NOT DONE: blocked on SCR-WMS-INB-01 §6 (WAITING_GM) and Q10/D-159 (the GM's review summary, then personal acceptance, `.golden-slice-accepted`) |
