...
| 0.18 | **RLS enabled on every operational table in 01/13/13B/019** (the patterns in 01 §11 and 13B §12 generalised) + **client isolation test** (ID tampering) | ✅ | 0.17 | **M** | SYSADMIN | **G7 returns 0** · User A returns zero rows from B's data on direct ID substitution, with no error | TODO — **pre-flight verified `<pending>` (2026-09-23)**: no 0.18 test exists yet on any branch or worktree. Preconditions that DO hold — `withContext()` is exported from `packages/db` (`packages/db/index.ts`) and sets all three GUCs the RLS predicates read; the five client-scoped tables (`wms.outbound_orders`, `tms.delivery_tasks`, `billing.invoices`, `sales.accounts`, `wms.skus`) are all defined in `01-Data-Model.sql`, so **no new migration is required**. Two real blockers — (1) `PGUSER` defaults to superuser `postgres`, which bypasses RLS unconditionally, and `database/schema/**` + `database/migrations/**` contain **zero `create role`**, so a non-superuser `NOBYPASSRLS` app role is a **G-01 prerequisite**; (2) G14's runner `pnpm test:isolation` is named in doc 40 Part F, `guards-run.sh` and `apply.sh` but is **defined in no `package.json`**. |
