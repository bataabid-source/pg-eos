...
**134 doc-38 tasks (v4.3: 0.6 → 0.6a / 0.6b, D-124; 5.3b added, D-167) + X.1–X.6 CONTINUOUS + 0.19 DEFERRED + DEFERRED-POST-PILOT rows (D-127)** — eight phases (0–7) + cross-cutting X-tasks; X.1–X.6 and 0.19 are counted inside the 133. No 18-phase roadmap, no separate database phase (BOOTSTRAP-v4 §8).
...
| 0.19 | Admin app shell: navigation, Decision Inbox, empty-state component, design system | 🤖 | 0.17 | **M** | SYSADMIN | Inbox renders; empty state shows "what's missing + owner" | READY — D-172 (visible pilot): lane 1 after 1.2; Phase-0 mechanism-only deferral lifted for 0.19 |
...
**Rows: 134** (expected 134 — `--check` fails otherwise; the five Staged rows above are never part of this count).
