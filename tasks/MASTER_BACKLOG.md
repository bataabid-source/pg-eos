...
| 2.11 | Outbound order: ten-condition check, FEFO allocation, pick sequence (A3) | 🤖 | 2.9, 1.8 | **1** | WH_MGR | Each of ten conditions has a failing test with the correct message | IN PROGRESS — **part 1 DONE @ `a96b013`** (lane 1): `CreateOutbound`/`RunOutboundChecks` (9 of 10 conditions — condition 10 BLOCKED, no schema source)/`ApproveOutbound`/`CancelOutbound`, no migration (0022 issued then withdrawn, column pre-existing at 13B:164-166); review PASS(0 findings) round 4 of 4, round 2→3 escalated to opus per D-117. **Part 2** (Allocate, GeneratePickList, allocated/partially_allocated cancel-release) remains open under this same row and the same `wms/process-outbound` lock |
