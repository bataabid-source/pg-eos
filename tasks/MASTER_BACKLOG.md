...
| 0.20 | Runbook v1 (deploy, rollback, restore, secrets rotation) — drafted as soon as 0.6a is green, sealed only after 0.8 restore succeeds | 🧑 (draft 🤖) | 0.6a, 0.6b, 0.8 | **A** | SYSADMIN | Eight procedures written and tested once | DONE @ `<this commit>` — docs/RUNBOOK.md v1, eight procedures (bootstrap, migration deploy, CI merge queue, backup, restore/rollback, guards re-verification, secrets Tier 0, incident quick reference), all previously exercised; §8 placeholders for 0.3/0.5/0.6b/0.7 (DEFERRED-POST-PILOT). Closes the Phase 0 gate fully |
...
| 1.4 | Pricing engine: exception → contract → segment → list → pending | 🤖 | 1.2 | **1** | CFO | Tiered pricing matches manual calc on 3 cases; unpriced event stays pending | DONE @ `<this commit>` — lane 1; read-only mechanism slice, no migration; consumed later by billing.billable_events once that module lands. |
...
| ID | Task | Type | Depends on | Lane | Owner | Acceptance | Status |
|---|---|---|---|---|---|---|---|
| 3.1 | `tms.vehicles`, documents, hard gate on expired docs | 🤖 | 0.9 | **1** | FLEET_MGR | Expired-doc vehicle cannot be assigned | **DONE @ `<this commit>`** — part 1 `e3137d5` (`RegisterVehicle` + documents, insert-only; PASS(0 open round 2)); part 2 `<this commit>` (`AssertVehicleAssignable`, the real INV-C4-1 hard gate, PASS(1 finding, 2 rounds)). **Master ruling (2026-09-26):** acceptance "Expired-doc vehicle cannot be assigned" is proven where it can be proven today — the gate refuses with a passing test; enforcing it AT the assignment write belongs to WBS 3.4 (`tms.delivery_tasks`/`tms.routes` writing `vehicle_id`, 01:890/907) and to the G-01 DB-level guard filed with part 1 (`tasks/backlog/MIGRATION-REQUEST-3.md`) — neither is this row's own scope. Lane 3 (D-184: doc-38 lane 1 row reassigned by the GM); `modules/fleet` created by new-slice.sh; no migration |
