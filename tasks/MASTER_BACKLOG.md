...
| 0.6a | CI gates ①–⑥ on GitHub Actions — **STATIC · UNIT · INTEGRATION · ACCEPTANCE · GUARDS · SECURITY (doc 36 §4-3)** (split from 0.6, D-124) | 🔧 | 0.4 | **M** | SYSADMIN | Every PR runs lint + boundaries + types (STATIC), domain unit tests (UNIT), integration tests against an ephemeral Postgres 16 service container (INTEGRATION), the acceptance-scenario placeholder (ACCEPTANCE, real scenarios arrive with the golden slice), `pnpm guards:run` (GUARDS, gate ⑤ runs G1–G18), and a secret + dependency scan (SECURITY); a red step on any of the six blocks merge. **D-133:** CI and the integration tests connect as role `pgeos_app` (no superuser, no `BYPASSRLS`); `entity_scope` is split into `USING` / `WITH CHECK` — closes the 0.18 carried-forward item | WAITING_GM — D-124 applied to doc 38 v4.2; dep 0.4 DONE @ 05674b5; acceptance carries D-133 (CI + integration tests as role pgeos_app, no superuser / BYPASSRLS; entity_scope split USING / WITH CHECK); D3 (2.3): unreviewed `.github/workflows/ci.yml` draft (gates ①②⑤ live; ③/④ blocked on the D-133 pgeos_app / entity_scope migration, not yet requested; ⑥ dependency-scan only, no secret scanner chosen) produced and ruled out of commit by pg-reviewer — carries a CI-service password literal `postgres`; moved to session scratchpad, not committed. Waits on GM: (1) approve issuing a migration number for the D-133 pgeos_app role / entity_scope USING/WITH CHECK split (pg-reviewer pre-migration review first), (2) choose a secret scanner |
