...
//
// Opens a transaction on a dedicated pool client, sets exactly the three session GUCs the schema
// reads back through platform.current_user_id() / platform.current_client_id() /
// platform.is_internal() (database/schema/01-Data-Model.sql:32-40), runs `fn` against a drizzle
...
//
// The three set_config(...) calls are parameterized via drizzle's `sql` tagged template — the
...
//
// A JS `null` for userId/clientId binds as SQL NULL; passing SQL NULL as set_config's second
