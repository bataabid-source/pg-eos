// WBS 0.19 — minimal i18n lookup helper. No library dependency for one screen (brief Master
// decision 8): a typed lookup over the `ar` file's key set, `ar` authored first and used as the
// fallback for a missing key in any other locale (never a blank string).
...
// Type guard for the locale `<select>`'s raw string value (fix round 1, finding 10): avoids an
// unchecked `as Locale` assertion at the call site.
